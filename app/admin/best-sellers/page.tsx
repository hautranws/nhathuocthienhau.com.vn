    ...
    <div className="min-h-screen bg-gray-50 p-6 font-sans">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Đang có <strong>{bestSellers.length}</strong> sản phẩm hiển thị ở mục bán chạy trang chủ
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4">
          <h2 className="font-bold text-gray-700">➕ Thêm sản phẩm vào danh sách bán chạy</h2>
                  ...
                  {searchResults.map((p) => (
                    <tr key={p.id} className="hover:bg-gray-50">
                      ...
                      <td className="p-3 text-center">
                        {p.is_best_seller ? (
                          <span className="bg-orange-100 text-orange-600 text-xs font-bold px-2 py-1 rounded-full">🔥 Đang bán chạy</span>
                        ) : (
                      ...
                      <td className="p-3 text-center">
                        <button
                          onClick={() => toggleBestSeller(p.id, p.is_best_seller)}
                        ...
                        >
                          {p.is_best_seller ? "✕ Bỏ bán chạy" : "🔥 Thêm bán chạy"}
                        </button>
                      </td>
                    </tr>
                  ))}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-5 border-b border-gray-100 flex items-center justify-between">
            <h2 className="font-bold text-gray-700">🔥 Danh sách bán chạy hiện tại ({bestSellers.length} sản phẩm)</h2>
          </div>

          {loading ? (
            <div className="p-10 text-center text-gray-500">⏳ Đang tải...</div>
          ) : bestSellers.length === 0 ? (
            <div className="p-10 text-center text-gray-400">Chưa có sản phẩm bán chạy nào. Hãy tìm và thêm ở trên.</div>
          ) : (
            <table className="w-full text-sm">
